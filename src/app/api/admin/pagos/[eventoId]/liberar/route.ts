...
export async function POST(
  ...
  try {
    ...
    const { data: evento, error: eventoError } = await supabaseAdmin
      .from('evento')
      .select('id, organizador_id, titulo, estado')
      .eq('id', eventoId)
      .single()
    ...
    if (evento.estado !== 'finalizado') {
      return NextResponse.json(
        { error: 'Solo puedes liberar retención cuando el evento está finalizado.' },
        { status: 400 }
      )
    }
    ...
    if (orderIds.length === 0) {
      return NextResponse.json({ message: 'No hay órdenes para liberar retención.', liberado: 0 }, { status: 200 })
    }
    ...
    if (paidOrderIds.length === 0) {
      return NextResponse.json({ message: 'No hay órdenes pagadas para liberar retención.', liberado: 0 }, { status: 200 })
    }

    const { data: pagos, error: pagosError } = await supabaseAdmin
      .from('pago')
      .select('id, monto_retenido, estado, estado_escrow, orden_id')
      .in('orden_id', paidOrderIds)
      .eq('estado', ADMIN_VALID_PAYMENT_STATUS)
    if (pagosError) throw pagosError

    const pagosPendientes = (pagos || []).filter((p) => p.estado_escrow === ADMIN_ESCROW_RETAINED)
    const montoLiberar = pagosPendientes.reduce((acc, p) => acc + toNumberSafe(p.monto_retenido), 0)
    if (pagosPendientes.length === 0 || montoLiberar <= 0) {
      return NextResponse.json({ message: 'No hay retención pendiente por liberar.', liberado: 0 }, { status: 200 })
    }
    ...
    const { error: updateError } = await supabaseAdmin
      .from('pago')
      .update({ estado_escrow: ADMIN_ESCROW_RELEASED })
      .in('id', idsToUpdate)
      .eq('estado_escrow', ADMIN_ESCROW_RETAINED)
    if (updateError) throw updateError

    if (evento.organizador_id) {
      const { data: balance } = await supabaseAdmin
        .from('balance_organizador')
        .select('saldo_disponible, total_ganado')
      ...
      if (!balance) {
        ...
        const saldoActual = Math.max(0, toNumberSafe(balance.saldo_disponible) - montoLiberar)
        await supabaseAdmin
          .from('balance_organizador')
          .update({ saldo_disponible: saldoActual, ultima_actualizacion: ahora })
          .eq('organizador_id', evento.organizador_id)
      }
    ...
    return NextResponse.json(
      {
        ...
        eventoId,
        pagosLiberadosCount: pagosPendientes.length,
        liberado: montoLiberar,
      },
      { status: 200 }
    )
  } catch (error: any) {
    console.error('ADMIN LIBERAR RETENIDO ERROR', error)
    return NextResponse.json({ error: error?.message || 'Error al liberar retención' }, { status: 500 })
  }
}
