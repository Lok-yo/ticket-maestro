              ...
              {menuAbierto && (
                <div className="absolute right-0 mt-2 w-56 bg-[#1a1030] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                  ...
                  <div className="py-1">
                    ...
                    {/* Organizadores y Admins */}
                    {(user.rol === 'organizador' || user.rol === 'admin') && (
                      <Link
                        href="/organizador"
                        onClick={() => setMenuAbierto(false)}
                        className="flex items-center gap-3 px-4 py-3 text-sm text-purple-300 hover:bg-white/10 hover:text-purple-200 transition font-medium"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
